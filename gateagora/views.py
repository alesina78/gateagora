...
def relatorios(request):
    ...
    return render(request, 'gateagora/relatorios.html', {
        ...
        'total_recebido_float': int(total_recebido * 100),
        'total_pendente_float': int(total_pendente * 100),
        'total_atrasado_float': int(total_atrasado * 100),
        'faturas_pagas_count':     faturas.filter(status='PAGO').count(),
        'faturas_pendentes_count': faturas.filter(status='PENDENTE').count(),
        'faturas_atrasadas_count': faturas.filter(status='ATRASADO').count(),
